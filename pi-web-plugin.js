...
async function fetchDocsTree(context) {
  ...
  try {
    ...
    // Pre-fetch titles for all files
    await preloadFileTitles(fileTree, context);
    return fileTree;
  } catch (err) {
...
async function fetchTreeEntries(context, path) {
  const url = `/api/machines/${encodeURIComponent(context.machine.id)}`
    + `/projects/${encodeURIComponent(context.workspace.projectId)}`
    + `/workspaces/${encodeURIComponent(context.workspace.id)}`
...
// ── Title extraction ──────────────────────────────────────────────────────
async function preloadFileTitles(tree, context) {
  fileTitles.clear();
  const promises = tree.map(async f => {
    const text = await readFileContent(f.path, context);
    if (text) {
      const title = extractTitle(text);
      fileTitles.set(f.path, title);
    }
  });
...
async function ensureMermaid() {
  ...
  if (mermaidLoading) {
    return new Promise((resolve) => {
      const check = setInterval(() => { if (mermaidReady) { clearInterval(check); resolve(true); } }, 100);
...
.doc-viewer-focus-panel .doc-viewer-layout{display:grid;grid-template-columns:minmax(260px,25%) minmax(0,1fr);height:100%;overflow:hidden}
.doc-viewer-focus-panel .doc-viewer-sidebar{border-right:1px solid var(--pi-border,#30363d);border-bottom:0;overflow-y:auto;min-height:0}
        ...
        actions: [
          {
            ...
            run: (context) => {
              const piWebUnstable = context?.piWebUnstable;
              if (piWebUnstable?.selectWorkspaceTool) {
                piWebUnstable.selectWorkspaceTool("doc-viewer:docs");
              }
            },
          },
        ],
...
function wireToolbar(context) {
  ...
  for (const toolbar of toolbars) {
    ...
    if (copyBtn && !copyBtn.innerHTML) copyBtn.innerHTML = iconSvg("copy");
...
async function saveEdit(context) {
  ...
  // Build a remote-safe Node.js heredoc command (same pattern as screenshot-paste)
  const script = `
const fs = require("fs");
const path = ${JSON.stringify(currentFile)};
...
`;
  ...
  try {
    const handle = await context.terminal.runCommand({
      title: `Save ${currentFile}`,
      command,
      open: false,
      metadata: { source: "doc-viewer", action: "save", path: currentFile },
    });
    const completed = await handle.completed;
    if (completed.status !== "succeeded" || completed.exitCode !== 0) {
      throw new Error(`Save command failed with exit code ${completed.exitCode}`);
    }
    // Invalidate caches
    renderedFiles.delete(currentFile);
    fileContents.set(currentFile, newContent);
    editBackup = null;
    editDraft = null;
    mode = "view";
    updateToolbarState();
