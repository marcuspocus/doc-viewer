...
flowchart TD
  ...
  Mounted --> Schedule[scheduleRender: tracked rAF + timeout]
  Schedule --> Load[Load docs tree and file content]
  Load --> Update[Update DOM with querySelectorAllDeep]
```

This keeps `render()` predictable while still allowing asynchronous file discovery, search, edit mounting, and Mermaid rendering. Pending scheduled callbacks are cancelled on every new `render()` and on `deactivate()` so they never accumulate.
...
Each discovered file's H1 title is extracted **lazily** the first time that file is read (for rendering or search), not in a bulk pass on tree load. The tree appears immediately without reading every file. If no H1 exists, the plugin falls back to a readable file-based label.
...
2. Save writes the file through pi-web's stable `context.files.writeFile()` API in the active workspace.
3. On success, caches are updated and the rendered view is refreshed.
4. Cancel drops the draft and restores view mode.

This approach works for local and remote workspaces because the write happens through the stable workspace file API rather than a browser-only file API or a shell command.
...
Focus mode is not browser fullscreen. It does not use the Fullscreen API, does not call `requestFullscreen`, and does not exit when the user presses `Esc`. In focus mode the divider between the tree and content is draggable and the chosen sidebar width persists in `localStorage`.
