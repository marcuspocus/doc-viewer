...
- **Remote-safe saves** — writes changes through the workspace terminal command API, so saves happen in the active workspace environment.
...
- Edit saves use `context.terminal.runCommand()` with a small Node.js script in the workspace.
